...
(deftest transpose-matrix
  ...
  (testing "Test transposing a vector"
    (is (= (matrix/transpose [[1 2 3]]) [[1]
                                         [2]
                                         [3]])))
  )
...
(deftest compute-product-by-inverse
  (testing "Test compute product by its inverse"
    (let [a [[3 -9 7 3]
             [3 -8 2 -9]
             [-4 4 4 1]
             [-6 5 -1 1]]
      ...
      (is (aeq (matrix/multiply c (matrix/invert b)) a))))
  )
